...
function httpMetricMiddlewareWrapper(options = {}) {
  ...
  return async function httpMetricMiddleware(ctx, next) {
    ...
    if (ctx.request.length) {
      httpRequestSizeBytes.labels(ctx.request.method, path, ctx.response.status).observe(ctx.request.length);
    }
    if (ctx.response.length) {
      httpResponseSizeBytes.labels(ctx.request.method, path, ctx.response.status).observe(ctx.response.length);
    }
    if (httpTimingEnabled(options)) {
    ...
    httpRequestsTotal.labels(ctx.request.method, path, ctx.response.status).inc();
  };
}
